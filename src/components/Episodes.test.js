...
test('Episodes render correctly', () => {
    ...
    expect(queryAllByTestId('episode-test')).toStrictEqual([])
    expect(queryAllByTestId('episode-test')).toHaveLength(0)

    rerender(<Episodes episodes={shows._embedded.episodes} />)

    // const epList = shows._embedded.episodes.length

    // expect(queryAllByTestId('episode-test')).toHaveLength(epList)
